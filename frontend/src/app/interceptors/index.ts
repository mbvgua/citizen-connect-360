...
export function TokenInterceptor(req: HttpRequest<unknown>, next:HttpHandlerFn): Observable<HttpEvent<unknown>> {
    if(req.url==='http://localhost:4000/auth/register' || req.url==='http://localhost:4000/auth/login'){
        // exclude all the routes that are not protected. PUT THEM IN ARRAY THEN USE CONTAINS
        // this will include all those from views incidents polls aichat and votes
        return next(req)
